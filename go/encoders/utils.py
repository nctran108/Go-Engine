...
def is_ladder(try_capture, game_state: GameState, candidate: Move, ladder_stones=None, recursion_depth=50):
    ...
    candidate: [Point] a move that potentially leads to escaping the ladder or capturing it, instance of Move
    ...
    if not game_state.is_valid_move(candidate) or not recursion_depth:
        return False
...
def is_candidate(game_state: GameState, move: Move, player):
...
def guess_ladder_stones(game_state: GameState, move: Move, escape_player):
    adjacent_strings = [game_state.board.get_go_string(nb) for nb in move.point.neighbors() if game_state.board.get_go_string(nb)]
    if adjacent_strings:
...
def detemine_escape_candidates(game_state: GameState, move: Move, capture_player):
    escape_candidates = move.point.neighbors()
    for other_ladder_stone in game_state.board.get_go_string(move.point).stones:
...
def count_liberties(game_state: GameState, move: Move):
    if game_state.board.get_go_string(move.point):
        return game_state.board.get_go_string(move.point).num_liberties
    return 0

def liberties(game_state: GameState, move: Move) -> list:
    return list(game_state.board.get_go_string(move.point).liberties)
