...
from go import encoders, goboard, kerasutil
...
class ValueAgent(Agent):
    ...
    def set_policy(self, policy):
        if policy not in ('eps-greedy', 'weighted'):
            raise ValueError('Policy must be either "eps-greedy" or "weighted"')
        self.policy = policy
    ...
    def select_move(self, game_state):
        ...
        for move in game_state.legal_moves():
            ...
            moves.append(move)
            board_tensor.append(board_tensor)
        ...
        # values of the next state from opponent's perspective
        opp_values = self.model.predict(board_tensor)
        opp_values = np.reshape(opp_values, len(moves))

        # Values from our perspective
        values = 1 - opp_values
    ...
    def rank_moves_eps_greedy(self, values):
        ...
        # ranks the moves from worst to best
    ...
    def train(self, experience, lr=0.1, batch_size=128):
        ...
        self.model.fit(experience.states, y,
